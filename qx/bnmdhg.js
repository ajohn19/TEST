...
async function getUserInfo() {
    return new Promise((resolve) => {
        let url = {
            url: `https://${host}/api/member/basic?member_id=${member_id}`,
            headers: getHeaders(),
        }
...
async function signInfo() {
    return new Promise((resolve) => {
        let url = {
            url: `https://${host}/api/sign-in/days?member_id=${member_id}`,
            headers: getHeaders(),
        }
        $.get(url, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (result.code == 200) {
                        ...
                        is_sign_in ? $.log(`今日已签到`) : await signIn();
                    } else {
