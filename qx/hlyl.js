...
脚本名称：活力伊利 签到
脚本作者：@ Levi
更新日期：2024.02.07 15:14:11

脚本兼容：Surge、QuantumultX、Loon、Shadowrocket、Node.js
只测试过surge和qx，其它环境请自行尝试
...
单账号&多账号：
1.将签到脚本拉取到本地
2.打开小程序，点击我的，点击复制ID，提示获取cookie成功则可以使用该脚本
3.关闭获取cookie脚本，防止产生不必要的mitm

------------------ Surge 配置 -----------------

[Script]
活力伊利获取token = type=http-request,pattern=https://zhls.qq.com/api/report?token=bifbdbfb7e1a6b4a93,requires-body=0,max-size=0,script-path=https://raw.githubusercontent.com/ajohn19/TEST/main/qx/hlyl.js
...
******************************************/
// env.js 全局
const $ = new Env("活力伊利小程序签到");
const ckName = "hlyl_data";
//-------------------- 一般不动变量区域 -------------------------------------
...
class UserInfo {
    ...
    async signin() {
        try {
            ...
            if (res?.code == 0 || res?.code == 5020) {
                $.signMsg = res?.data ? `累计${res?.data?.dailySignConfig?.giveDays}天,获得${res?.data?.dailySignConfig?.giveBonusPoints}币` : res?.msg
            } else {
                this.ckStatus = false;
            }
        } catch (e) {
    ...
    async userInfo() {
        try {
            ...
            debug(res, "查询积分");
            return {
                day: res?.data?.dailySignConfig?.giveDays,
                point: res?.data?.dailySignConfig?.bonusPoint,
                total: res?.data?.dailySignConfig?.giveBonusPoints
            }
        } catch (e) {
...
async function getNotice() {
    try {
        const urls = ["https://github.com/czy13724/Quantumult-X/blob/main/NAvailable/Declaration.json", "https://raw.githubusercontent.com/czy13724/Quantumult-X/main/NAvailable/Description.json"];
